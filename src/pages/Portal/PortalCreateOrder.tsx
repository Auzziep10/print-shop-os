...
export function PortalCreateOrder() {
  ...
  // Saved Carts Local Storage Fallback Helper
  ...
  const handleSubmitOrder = async (bypassProfileCheck: any = false) => {
    ...
    try {
      ...
      const resolvedItems = await Promise.all(orderItems.map(async (item) => {
         ...
         if (item.logoUrlTag) {
           const aspect = await getImageAspectRatio(item.logoUrlTag);
           const w = 2.5;
           const h = parseFloat((w * aspect).toFixed(2));
           artworks.push({ url: item.logoUrlTag, originalUrl: item.logoUrlTag, name: 'Size Tag Print', width: w, height: h, aspectRatio: aspect, quantity: totalQty });
         }
         ...
         return {
            ...
            originalFrontImage: item.originalFrontImage || null,
            originalBackImage: item.originalBackImage || null,
                              ...
                              {(() => {
                                ...
                                if (item.logoUrlFront || item.logoUrl || item.customizedFrontImage || lp.includes('front')) {
                                  ...
                                  if (w > 0) {
                                    ...
                                    sizeF = isLeftChest ? 'Small Front (4×4")' : isMedium ? 'Medium Front (7×9")' : 'Full Front (11×14")';
                                  }
                                ...
                                return (
                                  <div className="flex flex-col gap-1.5 mt-1.5">
                                    ...
                                    >
                                      ...
                                      <span>{item.customized ? "Adjust Saved Mockup" : "Edit Design Here"}</span>
                                    </button>
                                  </div>
                                );
                              })()}
