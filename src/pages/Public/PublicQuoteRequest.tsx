...
export function PublicQuoteRequest() {
  ...
  const [designerTab, setDesignerTab] = useState<'upload' | 'text' | 'clipart'>('upload');
  ...
  const handleRemoveGarmentFromLookbook = (idToRemove: string) => {
    if (flowMode === 'racks') {
      setRackItems(prev => prev.map(i => i.id === idToRemove ? { ...i, selected: false } : i));
      return;
    }
    setSelectedGarmentTypeItems(prev => prev.filter(i => i.id !== idToRemove));
  };
  ...
  const renderCustomBlankCard = (garmentTypeLabel?: string) => (
    ...
    >
      {/* Top Square Box matching mockup aspect ratio */}
  ...
  const resolveGarmentImage = (item: SanMarProduct, colorKey?: string, side: 'front' | 'back' | 'sleeve' = 'front') => {
    ...
    if (cKey && item.images) {
      ...
      if (imgSet) {
        if (typeof imgSet === 'string') return imgSet;
        if (typeof imgSet === 'object' && (imgSet as any)[side]) return (imgSet as any)[side];
        if (typeof imgSet === 'object' && (imgSet as any).front) return (imgSet as any).front;
      }
    }
    ...
    if (firstImgSet) {
      ...
      if (typeof firstImgSet === 'object' && (firstImgSet as any)[side]) return (firstImgSet as any)[side];
      if (typeof firstImgSet === 'object' && (firstImgSet as any).front) return (firstImgSet as any).front;
    }
  ...
  const getGarmentMockupImage = (
    ...
    // 2. Custom mockup for the rack/basics slot (storefront catalog uploaded mockup)
    if (side === 'front') {
    ...
    <div className="w-full">
      {step === 0 ? (
        ...
        <div className="min-h-screen bg-white font-sans pb-32 text-neutral-900 selection:bg-neutral-900 selection:text-white">
          ...
          {/* Main Content Area */}
          <div className="max-w-[1600px] mx-auto px-6 w-full mt-2 sm:mt-4">
              ...
              <div className="max-w-7xl mx-auto space-y-8 bg-white py-2">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-neutral-200/50">
                  ...
                  <div className="flex flex-wrap items-center gap-3 font-sans">
                    ...
                    >
                      Design Selections ({rackItems.filter(i => i.selected).length} Selected) <ArrowRight size={14} />
                    </PillButton>
                  </div>
                </div>
              ...
              <div className="bg-white/80 backdrop-blur-md rounded-3xl p-8 border border-neutral-200 shadow-sm max-w-7xl mx-auto space-y-8">
                ...
                <div className="pt-6 border-t border-neutral-200/50 flex justify-end">
                  <PillButton 
                    ...
                    className="gap-2"
                  >
                    Design Selections <ArrowRight size={14} />
                  </PillButton>
                </div>
              </div>
            )}
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-neutral-200/50">
                  ...
                  <div className="flex flex-wrap items-center gap-3 font-sans">
                    ...
                    <PillButton 
                      ...
                      className="gap-2 px-5 py-2 text-xs font-bold shadow-xs hover:scale-[1.02] active:scale-[0.98] transition-all"
                    >
                      Design Selections ({selectedGarmentTypeItems.length} Selected) <ArrowRight size={14} />
                    </PillButton>
                  </div>
                </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-neutral-200/50">
                      {matching.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-3 gap-y-8">
                          ...
                          {renderCustomBlankCard(GARMENT_TYPES.find(gt => gt.id === selectedGarmentType)?.label)}
                        </div>
                      )}
                ...
                (() => {
                  const displayItems: {
                    ...
                    : (() => {
                        ...
                        return fallbackItem ? [{ id: 'default-1', product: fallbackItem, color: selectedGarmentTypeColor || fallbackItem.colors[0] || 'Black', garmentType: selectedGarmentType }] : [];
                      })();
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                      {displayItems.map(item => {
                        ...
                        return (
                          <div 
                            key={item.id} 
                            className="bg-white border border-neutral-200/80 rounded-2xl overflow-hidden flex flex-col justify-between group shadow-3xs hover:shadow-md transition-all duration-350 relative"
                          >
                            {/* Remove button if more than 1 item */}
