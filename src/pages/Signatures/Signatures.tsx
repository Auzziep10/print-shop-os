...
export function Signatures() {
  ...
  const generateCompositeAndCopy = async () => {
    ...
    try {
      ...
      canvas.height = 150 * SCALE; // 120px banner + 30px overlap area underneath
      ...
      // Calculate cover for banner explicitly mapped to the new ultra-slim 120px banner height
      const bRatio = Math.max(800 / bannerImg.width, 120 / bannerImg.height);
      const bWidth = bannerImg.width * bRatio;
      const bHeight = bannerImg.height * bRatio;
      const bX = (800 - bWidth) / 2;
      const bY = (120 - bHeight) / 2;
      
      // Draw banner cleanly clipped to the strict 120px banner bounding box
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(24, 0);
      ctx.lineTo(776, 0);
      ...
      const centerX = 64 + 60; // Left padding 64 (8%), radius 60 => 124
      const centerY = 78; // Elevate center so exactly 85% is in the 120px banner, 15% outside
      ...
      ctx.beginPath();
      ctx.arc(centerX, centerY, 56, 0, Math.PI * 2); // 4px border
      ctx.clip();
      
      // Calculate cover for profile
      const pRatio = Math.max(112 / profileImg.width, 112 / profileImg.height);
                    ...
                    {compositeUrl ? (
                      ...
                      <tr>
                        <td colSpan={2} style={{ paddingBottom: '0' }}>
                           <div style={{ position: 'relative', width: '100%', aspectRatio: '800 / 150' }}>
